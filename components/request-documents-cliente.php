<?php
// $documents y $file_types vienen del controlador
$base_url = ROOT_URL . "/" . DOCUMENTS_DIR . "/";

// Iconos según tipo MIME
function getDocIcon($mime) {
    if (strpos($mime, 'image') !== false) return ['icon' => 'ki-picture', 'color' => 'success'];
    if (strpos($mime, 'pdf') !== false) return ['icon' => 'ki-document', 'color' => 'danger'];
    if (strpos($mime, 'word') !== false || strpos($mime, 'document') !== false) return ['icon' => 'ki-document', 'color' => 'primary'];
    if (strpos($mime, 'sheet') !== false || strpos($mime, 'excel') !== false) return ['icon' => 'ki-chart-simple', 'color' => 'success'];
    return ['icon' => 'ki-file', 'color' => 'info'];
}
?>
...
<div class="tab-list-container">
    <?php if (empty($documents)): ?>
        <div class="empty-state">
            ...
            <p class="empty-state-text">Cuando se suban documentos a tu solicitud aparecerán aquí</p>
        </div>
    <?php else: ?>
